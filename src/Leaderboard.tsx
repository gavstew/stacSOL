...
import { fmtAmount, shortPk, anonymousPseudonym } from './lib/format'
...
/**
 * Doxx opt-in toggle for the connected wallet. Renders the current state
 * (anonymous / doxxed-as-X) plus a one-tap button that triggers a
 * signMessage popup and POSTs to /api/doxx. After success, refreshes the
 * leaderboard so the row swaps between pseudonym and real address
 * immediately.
 */
function DoxxToggle({
  row,
  ...
  if (row.isDoxxed) {
    return (
      <div className="flex flex-col items-end gap-1 text-[10px]">
          ...
          {!editingName ? (
            ...
            <>
              ...
              <button
                type="button"
                disabled={busy}
                onClick={() => { setEditingName(false); setDraftName(row.displayName ?? '') }}
        ...
        {err && <span className="text-[var(--color-warn)]">{err}</span>}
      </div>
    )
  }
  // Not doxxed yet — single CTA.
  return (
    <div className="flex flex-col items-end gap-1 text-[10px]">
      <button
        type="button"
        ...
        className="px-2 py-0.5 rounded border border-[var(--color-ember)] text-[var(--color-ember)] hover:bg-[rgb(255_119_51_/_0.12)] uppercase tracking-[2px] disabled:opacity-40"
        title="sign a message so your wallet shows on the public leaderboard"
      >
...
/**
 * Renders a wallet identity for the leaderboard, respecting doxx state.
 *
 *  - Anonymous (default, even for the connected wallet's own row):
 *    shows a stable per-wallet pseudonym like `holder-7a2b1f` derived
 *    from the pubkey. No solscan link, no copy button — the real
 *    address never reaches the DOM, so screenshots of the leaderboard
 *    (or of the "you" sticky row at the top) don't leak the user's
 *    wallet. The own-row gets a small "you" badge so they can still
 *    spot which row is theirs.
 *  - Doxxed (opt-in via /api/doxx): real shortPk + solscan link + copy
 *    + optional display name. Applies the same whether it's the user's
 *    own row or someone else's — opting in is a global "show me" call.
 */
function WalletIdentity({
  ...
  if (!row.isDoxxed) {
    // Always anonymous when not opted in — including the user's own row.
    // The wallet address stays out of the DOM entirely so screenshots
    // can't leak it. The "you" badge lets the user identify their row
      ...
      <span
        ...
        title={
          isMe
            ...
            : 'anonymous holder — only the wallet owner can opt in to show their address'
        }
      >
  ...
  // Doxxed: render real link + display name + copy chip.
